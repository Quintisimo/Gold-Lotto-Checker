...
namespace Gold_Lotto_Checker_Stage_2 {
    ...
    class Program {
        ...
        static void Main() {
            ...
            int[] draw_number_array = DrawNumberCreation(drawNumbers);

            Welcome();
            ArrayCreator(lottoNumbers, draw_number_array);
        ...
        /// <summary>
        /// Generates a random number between 1 and 45, 
        /// then adds the number to the drawNumbers array. 
        /// </summary>
        /// <param name="draw_number_array">Empty array of length 8</param>
        /// <returns></returns>
        static int[] DrawNumberCreation(int [] draw_number_array) {
            int random_number = 0;

            for (int i=0; i < draw_number_array.Length; i++) {
                bool no_duplicate = true;

                while (no_duplicate) {
                    random_number = randomValue.Next(LOWER_LIMIT, UPPER_LIMIT);
                    no_duplicate = CheckNoDuplicate(draw_number_array, random_number);

                    if (!no_duplicate) {
                        draw_number_array[i] = random_number;
                    }
                }
            }
            return draw_number_array;
        }

        /// <summary>
        /// Checks if a number already exist in an array.
        /// </summary>
        /// <param name="array">Array of to be checked</param>
        /// <param name="number">Number to be checked to see if it already exists in the array</param>
        /// <returns></returns>
        static bool CheckNoDuplicate(int[] array, int number) {
